      ...
      <aside className="hidden lg:flex w-60 flex-col fixed inset-y-0 left-0 z-10"
        ...
        <div className="mx-3 mb-4 rounded-2xl overflow-hidden relative" style={{ height: 160 }}>
          ...
          <div className="absolute inset-0 p-4 flex flex-col justify-end"
            style={{ background: "linear-gradient(to top, rgba(30,26,46,0.9), transparent)" }}>
            <p className="text-xs text-white leading-snug">A quiet space for meaningful conversations.</p>
          </div>
        </div>
      </aside>
          ...
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex items-center gap-2 shrink-0">
              <Link href="/get-started"
                className="inline-flex h-9 items-center gap-1 justify-center rounded-full px-4 text-sm font-medium text-white transition-colors"
...
function TopicsIcon({ size = 20 }: { size?: number }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="3" /><path d="M12 2v3M12 19v3M4.22 4.22l2.12 2.12M17.66 17.66l2.12 2.12M2 12h3M19 12h3M4.22 19.78l2.12-2.12M17.66 6.34l2.12-2.12" /></svg>;
}
...
function AapunMarkLight({ size = 40 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 40 40" fill="none" aria-hidden>
      <circle cx="15" cy="20" r="11" fill="rgba(107,91,158,0.4)" stroke="rgba(107,91,158,0.8)" strokeWidth="1.5" />
